...
export const CatalogItemSchema = z.object({
  ...
  core_pattern: z.string().optional().describe("The fundamental loop or pattern"),
  source: z.string().optional().describe("Origin (karpathy/autoresearch, community, etc.)"),
...
export const TechniqueSpecSchema = z.object({
  search_strategy: SearchStrategyId.describe("How to explore the solution space"),
  ...
  domain_template: RecipeId.optional().describe("Optional domain recipe for scaffolding"),
});
...
export const BudgetSchema = z.object({
  max_iterations: z.number().optional().describe("Maximum experiment iterations"),
...
export const RiskPolicySchema = z.object({
  sandbox_only: z.boolean().default(false).describe("Must run in sandboxed environment"),
  requires_approval: z.boolean().default(false).describe("Human approval needed per iteration"),
  network_denied: z.boolean().default(true).describe("No network access during execution"),
  secrets_denied: z.boolean().default(true).describe("No access to env secrets"),
});

export const ConstraintsSchema = z.object({
  metric_floors: z.record(z.string(), z.number()).default({}).describe("Minimum acceptable values for metrics"),
  metric_ceilings: z.record(z.string(), z.number()).default({}).describe("Maximum acceptable values for metrics"),
});
...
export const ExperimentSpecSchema = z.object({
  target_artifact: z.string().describe("What is being optimized (prompt, code file, config, etc.)"),
  artifact_type: z.enum(["prompt", "code", "config", "content", "other"]).default("other"),
  technique: TechniqueSpecSchema.optional().describe("Composed technique specification"),
  recipe_id: RecipeId.optional().describe("Shorthand: use a predefined recipe"),
  mutation_strategy: z.string().describe("How changes are proposed (LLM edit, random, evolutionary, etc.)"),
  evaluator_command: z.string().describe("Command or script that returns a score (single float to stdout)"),
  metric_name: z.string().describe("Name of the primary metric (e.g., val_bpb, test_pass_rate)"),
  ...
  environment: z
    ...
    .default({}),
  stopping_conditions: z.array(StoppingCondition).default(["budget-exhaustion"]),
